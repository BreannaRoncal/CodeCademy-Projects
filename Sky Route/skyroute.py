...
You’ll be building out “SkyRoute,” a routing tool that uses breadth-first search, depth-first search, and Python dictionaries 
to accomplish this feat. For the purposes of this project, you can assume that it takes the same amount of time to get from 
each station to each of its connected neighboring stations.
...
def set_start_and_end(start_point, end_point):
  if start_point is not None:
    change_point = str(input("What would you like to change? You can enter 'o' for 'origin', 'd' for 'destination', or 'b' for 'both': ")).lower()
    if change_point == "b":
...
def get_start():
  start_point_letter = str(input("Where are you coming from? Type in the corresponding letter: ")).lower()
